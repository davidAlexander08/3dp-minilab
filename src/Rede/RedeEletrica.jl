...
@info "Preenchendo grafo da rede $(file_path_rede)"
...
for slack in lista_barras_slack
    ...
    for barra in lista_barras_da_ilha
        if barra.codigo != slack.codigo
            push!(vetorPotenciaCarga,  barra.carga)
        end
    end
    ...
    for linha in lista_linhas_da_ilha
        ...
        RHS = linha.MVAR +transpose(linhaMatrizSensibilidade)*vetorPotenciaCarga
