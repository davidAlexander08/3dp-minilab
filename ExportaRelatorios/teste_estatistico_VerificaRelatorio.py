...
arquivoRelatorioEstatisticoCSV = "estatisticasArvores.csv"
#df = pd.read_csv("estatisticasArvores.csv", sep=";")
df = pd.read_csv(pastaRelatorioEstatisticoCSV+ arquivoRelatorioEstatisticoCSV, sep=";")
...
dicionarioFalhas = {
    "mean":["FALHA_T_TEST", "Média Viol. LimInf", "Média Viol. LimSup"],
    "std":["FALHA_F-TEST", "Std Viol. LimInf", "Std Viol. LimSup"]
}
lista_relatorio = []
for tipo in tipos:
...
for tipo_falha in dicionarioFalhas:
    ...
    for est in estagios:
        ...
        for caso in casos:
            df_caso = df_resultados_est.loc[(df_resultados_est["CASO"] == caso)].reset_index(drop = True)
...
df_falhaTestesContagem.to_csv(pastaRelatorioEstatisticoCSV+"resultadoContagemFalhasEstagio.csv", index = False)

with open(pastaRelatorioEstatisticoCSV+"relatorioEstatisticoArvores.txt", "w") as report_file:
...
df_resultado_arvores.to_csv(pastaRelatorioEstatisticoCSV+"resultadoTotalFalhasArvores.csv", index = False)
