...
df_arvore_original <- read.csv("C:/Users/testa/Documents/git/3dp-minilab/Dissertacao/apresentacaoCarmen/caso_mini/avaliaArvores/arvoreAssimetrica_5_25_125_Backward/arvore_estudo.csv")
df_vazoes_original <- read.csv("C:/Users/testa/Documents/git/3dp-minilab/Dissertacao/apresentacaoCarmen/caso_mini/avaliaArvores/arvoreAssimetrica_5_25_125_Backward/vazoes_estudo.csv")
...
casos <- c("arvoreAssimetrica_5_25_125_Backward", "arvoreAssimetrica_5_25_125_Clustering", "arvoreSimetrica_5_25_125_Backward", "arvoreSimetrica_5_25_125_Clustering")
#casos <- c("Arvore1", "Arvore6" )
#casos <- c("Arvore6")
usinas <- unique(df_vazoes_original$NOME_UHE)
print(usinas)
lista_df_final <- list()
estagios <- c(4,3,2)
...
for (caso in casos) {
  df_arvore_reduzida <- read.csv(paste0("C:/Users/testa/Documents/git/3dp-minilab/Dissertacao/apresentacaoCarmen/caso_mini/",tipo,"/", caso, "/df_arvore_reduzida.csv"))
  df_vazoes_reduzida <- read.csv(paste0("C:/Users/testa/Documents/git/3dp-minilab/Dissertacao/apresentacaoCarmen/caso_mini/",tipo,"/", caso, "/df_vazoes_reduzida.csv"))
  for (est in estagios) {
    prob_original <- list()
    ...
    for (usi in usinas){
      lista_original <- list()
      for (no in unique(df_orig_est$NO)) {
        vazao <- df_vazoes_original[df_vazoes_original$NOME_UHE == usi & df_vazoes_original$NO == no, "VAZAO"][1]
      ...
      lista_red <- list()
      for (no in unique(df_red_est$NO)) {
        vazao <- df_vazoes_reduzida[df_vazoes_reduzida$NOME_UHE == usi & df_vazoes_reduzida$NO == no, "VAZAO"][1]
      ...
      #}
      if (is.na(sd(lista_original)) || is.na(sd(lista_red)) || sd(lista_original) == 0 || sd(lista_red) == 0) {
        cat("Teste não aplicado: Dataset contém NA ou é constante. orig: ", sd(lista_original, na.rm = TRUE), 
            " red: ", sd(lista_red, na.rm = TRUE), "\n")
      } else {
        # Apply the t-test if data is not constant and contains no NAs
        t_test_result <- t.test(lista_original, lista_red, var.equal = FALSE)
      }
...
nome <- paste0("C:/Users/testa/Documents/git/3dp-minilab/Dissertacao/apresentacaoCarmen/caso_mini/",tipo,"/","estatisticasArvores.csv")  # Correct string concatenation

write.csv2(df, nome, row.names = TRUE)
