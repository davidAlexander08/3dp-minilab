            ...
            for i_no in mapa_periodos[est].nos
                ...
                for ilha in lista_ilhas_eletricas ##QUESTAO AQUI, ESTA DECLARANDO TODOS OS GERADORES NA RESTRICAO DA ILHA, CONSIDERANDO QUE SO EXISTE UMA ILHA. CASO EXISTA MAIS DE UMA, E NECESSARIO CADASTRAR APENAS OS GERADORES DAQUELA ILHA NA RESTRICAO DE DEMANDA
                    ...
                    for linha in ilha.linhasAtivas[est]
                        ...
                        folga_rede_neg[(i_no.codigo, linha)] = @variable(m, base_name="sFluxoNeg_$(i_no.codigo)_$(linha.de.codigo)_$(linha.para.codigo)")
                    end
                end
            end
        end

        @objective( m, Min, sum(term.custo_geracao * gt_vars[(i_no.codigo, term.nome)] for i_no in lista_total_de_nos, term in lista_utes)
        + sum(0.01 * vert_vars[(i_no.codigo, uhe.nome)] for i_no in lista_total_de_nos, uhe in lista_uhes)+
        sum(sistema.deficit_cost*deficit_barra[(i_no.codigo, barra.codigo)] for i_no in lista_total_de_nos, barra in lista_barras) +
        sum(sistema.deficit_cost * deficit_vars[(i_no.codigo)] for est in 1:caso.n_est, i_no in mapa_periodos[est].nos))
        ...
        if rede_eletrica == 1
            ...
            for ilha in lista_ilhas_eletricas ##QUESTAO AQUI, ESTA DECLARANDO TODOS OS GERADORES NA RESTRICAO DA ILHA, CONSIDERANDO QUE SO EXISTE UMA ILHA. CASO EXISTA MAIS DE UMA, E NECESSARIO CADASTRAR APENAS OS GERADORES DAQUELA ILHA NA RESTRICAO DE DEMANDA
                for barra in ilha.barrasAtivas[est]
                    ...
                    def += valor_deficit
                    push!(df_barras,  (iter = it, est = est, node = i_no.codigo, prob = probabilidadeNo, ilha = ilha.codigo, codigoBarra = barra.codigo, generation = round(get(barra.potenciaGerada, (it, est, i_no.codigo), 0), digits = 1), demanda = barra.carga[est], potenciaLiquida = round(barra.potenciaLiquida[it, est, i_no.codigo], digits = 1), deficit= round(valor_deficit, digits = 1)))
                end
                for linha in ilha.linhasAtivas[est]
                    ...
                    push!(df_linhas,  (iter = it, est = est, node = i_no.codigo, prob = probabilidadeNo, ilha = ilha.codigo, codigoBarraDE = linha.de.codigo, codigoBarraPARA = linha.para.codigo, capacidade = linha.Capacidade[est], fluxo = round(linha.fluxoDePara[it, est, i_no.codigo], digits = 1) ,  folga = round(valor_folga_rede, digits = 1)))
                end
