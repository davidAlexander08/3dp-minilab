module Main
    ...
    alpha_vars = Dict{Int, JuMP.VariableRef}()
    ...
    function retornaModelo(est, no)
        ...
        alpha_vars[est] = @variable(m, base_name="alpha_$(est)")
        @constraint(m, alpha_vars[est] >= 0 )
    ...
    function imprimePolitica(text, est, it, miniIter, no)
        ...
        for term in lista_utes
            ...
            push!(df_termicas, (etapa = text, iter = it, miniIter = miniIter , est = est, node = no.codigo, prob = probabilidadeNo, nome = term.nome, usina = term.codigo, generation = round(geracao, digits = 1) , custo = term.custo_geracao, custoTotal = round(geracao, digits = 1)*term.custo_geracao))
        ...
        for uhe in lista_uhes
            ...
            push!(df_hidreletricas, (etapa = text, iter = it, miniIter = miniIter , est = est, node = no.codigo, prob = probabilidadeNo, nome = uhe.nome, usina = uhe.codigo, generation = geracao,
        ...
        if rede_eletrica == 1
            ...
            for ilha in lista_ilhas_eletricas ##QUESTAO AQUI, ESTA DECLARANDO TODOS OS GERADORES NA RESTRICAO DA ILHA, CONSIDERANDO QUE SO EXISTE UMA ILHA. CASO EXISTA MAIS DE UMA, E NECESSARIO CADASTRAR APENAS OS GERADORES DAQUELA ILHA NA RESTRICAO DE DEMANDA
                for barra in ilha.barrasAtivas[est]
                    ...
                    def += valor_deficit
                    push!(df_barras,  (etapa = text, iter = it, miniIter = miniIter , est = est, node = no.codigo, prob = probabilidadeNo, ilha = ilha.codigo, codigoBarra = barra.codigo, generation = round(get(barra.potenciaGerada, (it, est, no.codigo), 0), digits = 1), demanda = barra.carga[est], potenciaLiquida = round(get(barra.potenciaLiquida, (it, est, no.codigo), 0), digits = 1), deficit= round(valor_deficit, digits = 1)))
                end
                for linha in ilha.linhasAtivas[est]
                    ...
                    push!(df_linhas,  (etapa = text, iter = it, miniIter = miniIter , est = est, node = no.codigo, prob = probabilidadeNo, ilha = ilha.codigo, codigoBarraDE = linha.de.codigo, codigoBarraPARA = linha.para.codigo, capacidade = linha.Capacidade[est], fluxo = round(get(linha.fluxoDePara,(it, est, no.codigo),0), digits = 1) ,  folga = valor_folga_rede))
                end
                push!(df_balanco_energetico,  (etapa = text, iter = it, miniIter = miniIter ,est = est, node = no.codigo, prob = probabilidadeNo, ilha = ilha.codigo, Demanda = carga_estagio_ilha, GT = GeracaoTermicaTotal, GH = GeracaoHidreletricaTotal, Deficit = def, CustoPresente = custo_presente, CustoFuturo = JuMP.value(alpha_vars[est])))
            end
        else
            def = JuMP.value(deficit_vars[(est, no.codigo)])
            custo_presente += def*sistema.deficit_cost
            push!(df_balanco_energetico,  (etapa = text, iter = it, miniIter = miniIter ,est = est, node = no.codigo, prob = probabilidadeNo, ilha = 0, Demanda = sistema.demanda[est], GT = GeracaoTermicaTotal, GH = GeracaoHidreletricaTotal, Deficit = def, CustoPresente = custo_presente, CustoFuturo = JuMP.value(alpha_vars[est])))
        end
                ...
                for i_no in mapa_periodos[est].nos
                    ...
                    for miniIt in 1:caso.n_iter
                        ...
                        if est < caso.n_est && it > 1
                            ...
                            #@constraint(m, [iter = 1:caso.n_iter], alpha_vars[est] - sum(vf_vars[(est, i_no.codigo, uhe.nome)]*FCF_coef[est+1,uhe.codigo,iter] for uhe in lista_uhes)   >= FCF_indep[est+1,iter] ) #linha, coluna
                            for iter in caso.n_iter
                                @constraint(m, alpha_vars[est] - sum(vf_vars[(est, i_no.codigo, uhe.nome)]*FCF_coef[est+1,uhe.codigo,iter] for uhe in lista_uhes)   >= FCF_indep[est+1,iter] ) #linha, coluna
                            end
                        end
                        for ilha in lista_ilhas_eletricas
                    ...
                    custo_futuro = JuMP.value(alpha_vars[est])
                ...
                for est in caso.n_est:-1:1 
                    for i_no in mapa_periodos[est].nos  
                        ...
                        for uhe in lista_uhes
                            ...
                            println("est: ", est, " iter: ", it, " no: ", i_no.codigo," usi: ", uhe.codigo, " dual bal: ", dual_balanco_hidrico, " c_pres: ", custo_presente, " c_fut: ", custo_futuro, " Vi[i_no.codigo,i]: ", Vi[i_no.codigo,uhe.codigo], " FCF_coef: ", FCF_coef[est,uhe.codigo, it], " FCF_indep[est, it]: ", FCF_indep[est, it])
                        end
    ...
    for est_value in unique(df_balanco_energetico.est)
        ...
        for iteracao in unique(subset.iter)
            ...
            for no in unique(subsubset.node)
                ProbGT += (subsubset[(subsubset.node .== no), "GT"][1])*(subsubset[(subsubset.node .== no), "prob"][1])
                ProbGH += (subsubset[(subsubset.node .== no), "GH"][1])*(subsubset[(subsubset.node .== no), "prob"][1])
                ProbDeficit += (subsubset[(subsubset.node .== no), "Deficit"][1])*(subsubset[(subsubset.node .== no), "prob"][1])
                ProbCustoPresente += (subsubset[(subsubset.node .== no), "CustoPresente"][1])*(subsubset[(subsubset.node .== no), "prob"][1])
                ProbCustoFuturo += (subsubset[(subsubset.node .== no), "CustoFuturo"][1])*(subsubset[(subsubset.node .== no), "prob"][1])
                Demanda += (subsubset[(subsubset.node .== no), "Demanda"][1])*(subsubset[(subsubset.node .== no), "prob"][1])
            end
