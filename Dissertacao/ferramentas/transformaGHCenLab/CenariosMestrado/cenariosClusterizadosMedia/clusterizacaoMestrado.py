...
caminho = "C:\\Users\\testa\\Documents\\git\\3dp-minilab\\ferramentas\\transformaGHCenLab\\CenariosMestrado\\cenariosClusterizadosMedia\\"
df_resultante = pd.read_parquet(caminho+"media_cenarios_GhcenSemanais_4_1540.parquet", engine = "pyarrow")
...
df_cenariosFinais.rename(columns={"previsao_incremental": "valor"}, inplace=True)
print(df_cenariosFinais)
df_cenariosFinais.to_csv(caminho+'cenarios_semanais.csv', index=False)
df_cenariosFinais.to_parquet(caminho+'cenarios_semanais.parquet', index=False)
fim = time.time()
